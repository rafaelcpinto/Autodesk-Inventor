Sub Main()
	...
	oPartComp.Parameters.Item("VALORES_OTIMOS").Value =
		calc.Otimiza(XL, XU, varOtmz, valorOtimo)

	oPartDoc.Update()

	oPartComp.Parameters.Item("MASSA_CALCULADA").Value =
		oPartComp.MassProperties.Mass

	oPartComp.Parameters.Item("ERRO").Value =
		Math.Abs(valorOtimo - oPartComp.MassProperties.Mass)
End Sub
...
Public Class calculos
	...
	Function Otimiza(
		...
		XU As Double,
		varOtmz As String,
		valorOtimo As Double
	) As Double

		Dim R As Double
		Dim d As Double
		Dim iteracoes As Integer
		Dim x1 As Double
		Dim x2 As Double
		Dim f1 As Double
		Dim f2 As Double
	...
	Function CALCMASS(A As Double) As Double
		...
		Try
			...
			oPartDoc.Update()
		Catch
			' Mantido sem interrupção para preservar o comportamento
			' original da regra iLogic.
		End Try
